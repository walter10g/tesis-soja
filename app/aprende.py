...
from model import ResNet18SegmentationModel
import numpy as np
import os
...
def main():
    ...
    # DataLoaders con `num_workers=0` para evitar conflictos
    train_loader = DataLoader(
        train_dataset,
        batch_size=batch_size,
        sampler=train_sampler,
        num_workers=0,  # Cambiar a 4 para rendimiento una vez que funcione
        pin_memory=True
    )
    ...
    # Cargar modelo
    model = ResNet18SegmentationModel(num_classes=4).to(device)
    ...
    # Congelar el backbone para entrenamiento inicial
    ...
    # Descongelar todo el modelo para ajuste fino
    for param in model.backbone.parameters():
    ...
    print("\nFase 2: Ajuste fino")
    model = train_and_evaluate(model, train_loader, test_loader, optimizer, criterion, scheduler, num_epochs // 2, model_path)
